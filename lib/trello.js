import { cleanStringify, getValuesFromHash, isCommonChecklistName, stringify, toCsvValue } from './utils.js'
...
function getChecklistItemsAsCards (boardDataChecklists, card) {
  const checklists = boardDataChecklists.filter(checklist => checklist.idCard === card.id)
  return checklists.flatMap(checklist => checklist.checkItems.map(checkItem => {
    const labels = checkItem.labels ?? []

    if (!isCommonChecklistName(checklist.name)) {
      labels.push({ name: checklist.name })
    }
...
function getCardToCsvLines (
  ...
  return function cardToCsvLines (card) {
    ...
    return [
      cardLine,
      ...getChecklistItemsAsCards(boardDataChecklists, card).flatMap(cardToCsvLines)
    ]
  }
}
...
function cardToString (boardDataLists, boardDataMembers, boardDataCustomFields, card) {
  ...
  const contentHandlers = {
    ...
    desc: (content) => typeof content === 'string' ? [content.trim()] : [undefined],
    points: () => [],
    membersFullName: () => [],
    labels: (content) => {
